...
plot(res12$cumul_calib, standardize=F)
res12$cumul_calib$details
res12$HL
```
...
plot(res22$cumul_calib, standardize=F)
res22$cumul_calib$details
res22$HL
```
...
#### Small dev, small val (11)

```{r}
plot(res11$calib$calibration_plot)
plot(res11$cumul_calib)
res11$cumul_calib$details
res11$HL
```

One-part test interpretation: Location was at index `r res11$cumul_calib$loc`, corresponding to predicted value `r val_data1[res11$cumul_calib$loc,'pi1']` which is at `r round(100*res11$cumul_calib$loc/dim(val_data1)[1], 1)` percentile.

#### full dev, small val (21)

```{r}
plot(res21$calib$calibration_plot)
plot(res21$cumul_calib)
res21$cumul_calib$details
res21$HL
```

One-part test interpretation: Location was at index `r res21$cumul_calib$loc`, corresponding to predicted value `r val_data1[res21$cumul_calib$loc,'pi2']` which is at `r round(100*res21$cumul_calib$loc/dim(val_data1)[1], 1)` percentile.
